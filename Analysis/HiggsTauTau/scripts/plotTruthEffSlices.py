...
for i in xrange(1, hist.GetNbinsY()+1):
    ...
    for j, src in enumerate(args.input):
        splitsrc = src.split(':')
        if splitsrc[0].find("gen")>=0:
            print "gr_%s_projx_%i"%(splitsrc[1], i)
            slice = ROOT.TGraphAsymmErrors
            file = ROOT.TFile(splitsrc[0])
            slices.append(file.Get("gr_%s_projx_%i"%(splitsrc[1], i)).Clone())
            file.Close()
        else:
            slices.append(hists[j].ProjectionX('%s_projx_%i' % (hists[j].GetName(), j), i, i))
        
        if len(splitsrc) >= 3:
            settings = {x.split('=')[0]: eval(x.split('=')[1]) for x in splitsrc[2].split(',')}
            plot.Set(slices[-1], **settings)
        if len(slices)==1:
            slices[-1].Draw()
        else:
            slices[-1].Draw('SAME')
        legend.AddEntry(slices[-1])
        #slices.append(slice)
